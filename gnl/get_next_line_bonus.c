...
/*   Updated: 2022/06/14 15:56:46 by jgarcia          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	if (((fd < 0) || (read(fd, &result, 0) < 0)) != 0)
		return (NULL);
	head[fd] = read_and_save(fd, head[fd]);
	if (head[fd] == NULL)
		return (NULL);
	size = list_len(head[fd]);
	result = malloc (sizeof(char) * (size + 2));
	if (result)
		result = extract_line(head[fd], size, result);
	if (result == NULL)
		return (NULL);
	head[fd] = list_clean(head[fd], 0, 0);
	if (result[0] == '\0')
	{
		list_free(head[fd]);
		free (result);
